...
class RunManager:
    ...
    def __create_metrics_df_with_retry(self, run_path: str, max_retries=3, initial_timeout=30):
    ...
    def __create_metrics_df(self, run_path: str) -> pl.DataFrame:
        try:
            ...
            metrics_df = pl.from_dataframe(run.history(stream="events", samples=20))
            if len(metrics_df) <= 1:
                return pl.DataFrame()

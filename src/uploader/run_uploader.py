...
class RunUploader:
    def __init__(self, new_runs_df, date_range: List, companies_to_refresh: List = None):
        ...
        self.companies_to_refresh = companies_to_refresh

    def process_and_upload_runs(self):
        old_runs_df = ArtifactHandler.read_dataset()
        all_runs_df = DataProcessor.combine_df(
            new_runs_df=self.new_runs_df, 
            old_runs_df=old_runs_df,
            companies_to_refresh=self.companies_to_refresh
        )
